    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void geom(triangle v2g i[3], inout TriangleStream<g2f> stream)
            {
                ...
                // 草の高さと向き，風の強弱の基準となる数値を計算.
                float height = (i[0].hei.r + i[1].hei.r + i[2].hei.r) * 0.33333;
                float rot = (i[0].rot.r + i[1].rot.r + i[2].rot.r) * 0.33333;
                float wind = (i[0].wind.r + i[1].wind.r + i[2].wind.r) * 0.33333;

                // 草の中心部分と，生えていく方向を計算.
                float4 center = ((p0 + p1 + p2) * 0.33333);
                float4 normal = float4(((n0 + n1 + n2) * 0.33333).xyz, 1.0);

                // 下部・中心部・上部についての高さと幅を計算し，それをもとに座標を求める.
                ...
                rot = rot - 0.5;
                float4 dir = float4(normalize((p2 - p0) * rot).xyz, 1.0);
                ...
                o[2].color = lerp(_BottomColor, _TopColor, 0.33333);

                o[3].pos = center + dir * middleWidth + normal * bottomHeight;
                o[3].color = lerp(_BottomColor, _TopColor, 0.33333);

                // Middle to top
                o[4].pos = o[3].pos - dir * topWidth + normal * middleHeight;
                o[4].color = lerp(_BottomColor, _TopColor, 0.66666);

                o[5].pos = o[3].pos + dir * topWidth + normal * middleHeight;
                o[5].color = lerp(_BottomColor, _TopColor, 0.66666);
